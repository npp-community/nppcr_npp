	...
	while ( sc.More() ) {
		...
		if (sc.atLineEnd) {
			...
			if ( sc.state > SCE_RSP_REBOL_DEFAULT && sc.state != SCE_RSP_REBOL_STRING &&
				...
				sc.state != SCE_RSP_REBOL_TAGSTRING && sc.state != SCE_RSP_HTML_COMMENT)
				sc.SetState(SCE_RSP_REBOL_DEFAULT);
		...
		if (sc.state < SCE_RSP_REBOL_DEFAULT) {
			if (sc.ch == '<') {
				...
				if (sc.chNext == '!' && sc.GetRelative(2) == '-' && sc.GetRelative(3) == '-') {
					sc.SetState(SCE_RSP_HTML_COMMENT);
					sc.Forward(5);
					continue;
				}
			}
		...
		if (sc.state == SCE_RSP_HTML_COMMENT) {
			if (sc.ch == '-' && sc.chNext == '-' && sc.GetRelative(2) == '>') {
				sc.Forward(3);
				sc.SetState(SCE_RSP_REBOL_DEFAULT);
			}
			sc.Forward();
